...
class LayoutPostprocessor:
    ...
    def __init__(self, cells: List[TextCell], clusters: List[Cluster], page_size: Size, page_number: int):
    ...
    def postprocess(self) -> Tuple[List[Cluster], List[TextCell]]:
        ...
        # _log.debug("Before line number filtering, regular_clusters count: %d", len(self.regular_clusters))

        # Remove line-number clusters from regular clusters
        # self.regular_clusters = self._filter_line_number_clusters(self.regular_clusters)

        # _log.debug("After line number filtering, regular_clusters count: %d", len(self.regular_clusters))
...
#--------------------------------------------------------------
        # Combine clusters
        combined_clusters = self.regular_clusters + self.special_clusters # new 

        # Merge vertically adjacent formulas here:
        combined_clusters = self._merge_vertically_adjacent_formulas(combined_clusters) # new 
        ...
        #  Remove tables containing page footers
        combined_clusters = self._filter_tables_containing_page_footer(combined_clusters)

        # Use already merged clusters
        final_clusters = self._sort_clusters(combined_clusters, mode="id")
...
#--------------------------------------------------------------
    # def _has_table_label(self, cluster: Cluster, table_labels: Set[str]) -> bool:
    #     """
    #     Recursively checks if cluster or any of its children have a table-like label.
    #     """
    #     if str(cluster.label).lower() in table_labels:
    #         return True
    #     if hasattr(cluster, 'children') and cluster.children:
    #         return any(self._has_table_label(child, table_labels) for child in cluster.children)
    #     return False
    ...
    def _filter_tables_containing_page_footer(self, clusters: List[Cluster], 
        ...
        for cluster in clusters:
            if cluster.label in large_labels:
                ...
                # Only consider large area clusters
                if cluster_area_ratio >= min_area_ratio:
                    # Collect all cells from this cluster and its children
                    ...
                    if len(all_cells) < min_cells_threshold or cell_density < min_density_threshold:
                        _log.debug(
                            f"[Page {self.page_number}] Reclassifying cluster {cluster.id} from {cluster.label.value} to TEXT "
                            f"(area ratio: {cluster_area_ratio:.2f}, cells: {len(all_cells)}, density: {cell_density:.6f})."
                        )
                        cluster.label = DocItemLabel.TEXT
                        reclassified_count += 1
                    else:
                        _log.debug(
                            f"[Page {self.page_number}] Keeping cluster {cluster.id} as {cluster.label.value} "
                            f"(area ratio: {cluster_area_ratio:.2f}, cells: {len(all_cells)}, density: {cell_density:.6f})."
                        )

        _log.debug(f"[Page {self.page_number}] Total clusters reclassified from large size labels to TEXT: {reclassified_count}")
        return clusters

    def _collect_all_cells(self, cluster: Cluster) -> List[TextCell]:
        """
        Recursively collect all cells from cluster and its child clusters.
        """
        cells = cluster.cells[:]
        for child in getattr(cluster, 'children', []):
            cells.extend(self._collect_all_cells(child))
        return cells
...
#--------------------------------------------------------------
    ...
    def _extract_formula_number(self, cluster: Cluster) -> Optional[str]:
        """
        Extracts a candidate formula number (letters/digits) from the cluster's cell text.
        Searches anywhere in the text for a pattern in parentheses.
        Only returns the candidate if its length is <= max_len (default 4).
        Example matches: (A1), (10b), (5), (123), etc.
        """
        import re
        pattern_anywhere = r'\(\s*([A-Za-z0-9]+)\s*\)'
        max_len = 4  # change this as needed

        for cell in cluster.cells:
            text = cell.text.strip()
            matches = re.findall(pattern_anywhere, text)
    ...
    def _merge_vertically_adjacent_formulas(
        ...
        """
        Robustly merge vertically adjacent FORMULA clusters using Union-Find.
        ...
        # print(f"Dynamic vertical threshold: {vertical_threshold}, Overlap threshold: {horizontal_overlap_threshold}")
        _log.debug(f"Dynamic vertical threshold: {vertical_threshold}, Overlap threshold: {horizontal_overlap_threshold}")
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                # print(f"Checking merge:\n  Current: {c1.id}, Next: {c2.id}")
                _log.debug(f"Checking merge:\n  Current: {c1.id}, Next: {c2.id}")

                # print(f"  Vertical gap: {vertical_gap}, Overlap ratio: {overlap_ratio}")
                _log.debug(f"  Vertical gap: {vertical_gap}, Overlap ratio: {overlap_ratio}")


                left_diff = abs(c1.bbox.l - c2.bbox.l)
                right_diff = abs(c1.bbox.r - c2.bbox.r)
                # print(f"  Left boundary diff: {left_diff}, Right boundary diff: {right_diff}")
                _log.debug(f"  Left boundary diff: {left_diff}, Right boundary diff: {right_diff}")

                alignment_factor = compute_alignment_factor(c1, c2)
                # print(f"  Alignment factor (ratio): {alignment_factor}")
                _log.debug(f"  Alignment factor (ratio): {alignment_factor}")
                ...
                # print(f"  -> c1 {c1.id} formula number: {num1}, c2 {c2.id} formula number: {num2}")
                _log.debug(f"  -> c1 {c1.id} formula number: {num1}, c2 {c2.id} formula number: {num2}")


                # Branch 4) If both have numbers and differ, skip
                if num1 and num2 and num1 != num2:
                    # print(f"  -> Not merging clusters {c1.id} and {c2.id} due to different formula numbers: {num1} vs {num2}")
                    _log.debug(f"  -> Not merging clusters {c1.id} and {c2.id} due to different formula numbers: {num1} vs {num2}")
                ...
                if num1 is None and num2 is None:
                    if vertical_gap > 3:  # Change this constant if needed
                        # print(f"  -> Not merging clusters {c1.id} and {c2.id} because vertical_gap {vertical_gap} > 3 with no numbers")
                        _log.debug(f"  -> Not merging clusters {c1.id} and {c2.id} because vertical_gap {vertical_gap} > 3 with no numbers")
                ...
                # Branch 3) One number missing => require vertical_gap ≤5 and overlap ≥0.95
                    ...
                    if vertical_gap > 12.8:
                        # print(f"  -> Not merging {c1.id} and {c2.id} (one missing, gap {vertical_gap} > 12.8)")
                        _log.debug(f"  -> Not merging {c1.id} and {c2.id} (one missing, gap {vertical_gap} > 12.8)")
                ...
                if overlap_ratio >= required_overlap:
                    # print(f"  -> Merging clusters {c1.id} and {c2.id} with vertical gap {vertical_gap} and overlap ratio {overlap_ratio}")
                    _log.debug(f"  -> Merging clusters {c1.id} and {c2.id} with vertical gap {vertical_gap} and overlap ratio {overlap_ratio}")
    ...
    def _adjust_cluster_bboxes(self, clusters: List[Cluster]) -> List[Cluster]:
        ...
        for cluster in clusters:
            ...
            elif cluster.label == DocItemLabel.FORMULA:
                # Expanded bounding box with padding specifically for formulas
                top_padding = 2  # pixels to expand left/right, adjust as needed
                bottom_padding = 1    # pixels to expand top/bottom, adjust as needed
                cluster.bbox = BoundingBox(
                    l=cells_bbox.l,  # No horizontal padding
                    t=max(cells_bbox.t - top_padding, 0),  # Prevent going above the page boundary
                    r=cells_bbox.r,  # No horizontal padding
                    b=min(cells_bbox.b + bottom_padding, self.page_size.height),  # Stay within page bounds
                )
