    ...
    def get_text_in_rect(self, bbox: BoundingBox) -> str:
        ...
        for i, cell in enumerate(self._dpage.textline_cells):
            ...
            if overlap_frac > 0.5:
                if len(text_piece) > 0:
                    text_piece += " "
                text_piece += self.normalize_text(cell.text)
...
# MODIFIED
    def get_text_cells(self) -> Iterable[TextCell]:
        ...
        # Convert cells to top-left origin in place
        for tc in self._dpage.textline_cells:
            tc.to_top_left_origin(page_size.height)

        # Filter out unwanted line numbers
        filtered_cells = [
            cell for cell in self._dpage.textline_cells
            if not self._is_left_margin_line_number(cell, page_size)
        ]

        return filtered_cells  # Correctly returns List[TextCell]

    def _is_left_margin_line_number(self, cell: TextCell, page_size: Size) -> bool:
        """
        Identify if a given cell is a line number in the left margin.
        Adjust threshold parameters based on empirical observations.
        """
        LEFT_MARGIN_THRESHOLD = 0.08  # ~8% of page width
        MIN_HEIGHT_THRESHOLD = 5      # minimum height for filtering
        MAX_WIDTH_THRESHOLD = page_size.width * LEFT_MARGIN_THRESHOLD

        bbox = cell.rect.to_bounding_box()

        # Conditions to flag as line number:
        # - located near the left margin
        # - very narrow horizontally
        # - sufficiently tall vertically (to avoid small footnotes or similar)
        is_small_horizontal = bbox.width < MAX_WIDTH_THRESHOLD
        is_within_left_margin = bbox.l < MAX_WIDTH_THRESHOLD
        is_small_vertical = bbox.height < MIN_HEIGHT_THRESHOLD

        return is_within_left_margin and is_small_horizontal and not is_small_vertical
#------------------------------------------------------------------------------------
