...
class CodeFormulaPredictor:
    ...
    def predict(
        ...
        We'll do it in a batch to minimize overhead, but each snippet has
        its own prompt in "text=" and the same image in "images=" is a bit tricky.
        """
        ...
        # 2) Convert each image to a tensor
        # The docling pipeline gave us PIL images. We'll do a standard resize + normalization:
            ...
            # 3) Build docling chat messages for each snippet
            ...
            for i, (img, lbl) in enumerate(zip(processed_pil_images, labels)):
                ...
                # We'll store a single-image list for docling's image handling
                snippet_images.append([img])

            # 4) For each snippet, we do the docling approach:
            #    processor.apply_chat_template([...], add_generation_prompt=True)
            #    Then pass images => [imglist]
            #    We'll batch them if you want. But let's do them separately for clarity.
            results = []
            ...
            # images_batch = [[img] for img in processed_pil_images]
            ...
            for i in range(output_ids_list.size(0)):
                ...
                if token_count >= max_new_tokens:
                    _log.warning("Output %d reached the maximum token limit (%d tokens).", i, token_count)
                else:
                    _log.debug("Output %d generated %d tokens.", i, token_count)
