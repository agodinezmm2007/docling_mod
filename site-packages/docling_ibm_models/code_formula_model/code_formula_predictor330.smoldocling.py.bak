...
from docling_core.types.doc import DoclingDocument, TextItem, CodeItem
...
class CodeFormulaPredictor:
    ...
    def _get_prompt(self, label: str) -> str:
        ...
        if label == "code":
            user_text = "Convert code to text."
        elif label == "formula":
            user_text = " Convert formula to LaTeX"
        else:
    ...
    def predict(
        ...
        if hasattr(output, "sequences_scores") and output.sequences_scores is not None:
            ...
            for beam_i, score in enumerate(scores):
                _log.info("Snippet %d, beam %d beam score: %.4f", i, beam_i, score)
        else:
        ...
        for beam_idx in range(output_ids_list.size(0)):
            ...
            if snippet_label == "formula":
                eq_data = [{"latex": text}]
                # Pass convert_to_markdown=False to disable conversion to Markdown-friendly LaTeX.
                converter = LatexToMarkdown(eq_data, convert_to_markdown=False)
                converted = converter.convert_all()
                if converted:
                    text = converted[0]["markdown"]  # e.g. "$$ f_{x} $$" if conversion were enabled
